...
func TestGetKeyVersionFromKID(t *testing.T) {
	tests := []struct {
		desc       string
		kid        string
		expectedID string
		...
		{
			desc:       "valid kid",
			kid:        "https://testkv.vault.azure.net/keys/key1/4cbaf3c6fdaa427ab1c06985e1a801f0",
			expectedID: "4cbaf3c6fdaa427ab1c06985e1a801f0",
		},
	}

	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			id := getKeyVersionFromKID(test.kid)
			if id != test.expectedID {
				t.Fatalf("expected id: %s, got: %s", test.expectedID, id)
			}
		})
	}
}
